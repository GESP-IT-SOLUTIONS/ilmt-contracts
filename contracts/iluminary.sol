...
contract IluminaryToken is ERC20, Ownable, Pausable {
    ...
    constructor() ERC20("Iluminary Token", "ILMT") {
        ...
        */
        ...
        _mint(address(this), 6885000 * 10 ** 18); 

        /// PRIVATE
        _mint(0x318cBF186eB13C74533943b054959867eE44eFFE, 3240000 * 10 ** 18);
        _mint(address(this), 12960000 * 10 ** 18); 
        ...
        /// TEAM
        _mint(address(this), 32400000 * 10 ** 18); 
        vestingSchedules[address(this)].tokensPerCliff = [972000 * 10 ** 18, 375000000000000000000000, 750000000000000000000000];
        vestingSchedules[address(this)].cliffs = [block.timestamp + 365 days, block.timestamp + 180 days, block.timestamp + 270 days];

        /// ADVISORS
        _mint(address(this), 5400000 * 10 ** 18); 
        vestingSchedules[address(this)].tokensPerCliff = [375000000000000000000000, 375000000000000000000000, 750000000000000000000000];
        vestingSchedules[address(this)].cliffs = [block.timestamp + 90 days, block.timestamp + 180 days, block.timestamp + 270 days];

        /// MARKETING
        _mint(0x318cBF186eB13C74533943b054959867eE44eFFE, 1350000 * 10 ** 18);
        _mint(address(this), 12150000 * 10 ** 18); 
        vestingSchedules[address(this)].tokensPerCliff = [375000000000000000000000, 375000000000000000000000, 750000000000000000000000];
        vestingSchedules[address(this)].cliffs = [block.timestamp + 90 days, block.timestamp + 180 days, block.timestamp + 270 days];
    ...
    function addVestingSchedule(address[] memory receivers, uint256[] memory tokens, uint256[] memory cliffs) external onlyOwner whenNotPaused {
    ...
    function claimVestedTokens(address claimer) external {     
        ...
        _transfer(address(this), claimer, availableTokens);     
