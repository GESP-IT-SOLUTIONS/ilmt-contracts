...
contract MultichainTokenTransfers is CCIPReceiver, OwnerIsCreator {
    ...
    uint256 public feePercentage = 0; // The % amount that is taken from each transaction (/10000). 500 = 5%
    ...
    mapping(address => address) public allowlistedTokens;
    ...
    {
        require(token == address(s_token), "Token is not supported");
        ...
        returns (bytes32 messageId)
    {
        ...
        console.log(router.isChainSupported(13264668187771770619));
        ...
        IERC20(token).forceApprove(address(router), amount - feeAmount);
        ...
        ) // Make sure source chain and sender are allowlisted
    {
        ...
        (
            ...
            s_lastReceivedTokenAddress,
            s_lastReceivedTokenAmount
        ) = abi.decode(any2EvmMessage.data, (address, address, uint256)); // abi-decoding of the sent text

        IERC20MintBurn(address(s_token)).mint(
            s_lastReceivedReceiverWallet,
            s_lastReceivedTokenAmount
        );
    ...
    function _buildCCIPMessage(
        ...
        // Create an EVM2AnyMessage struct in memory with necessary information for sending a cross-chain message
        bytes memory data = abi.encode(_receiverWallet, _token, _amount);
        return
